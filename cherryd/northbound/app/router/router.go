...
func (r *Router) Init() error {
	// FIXME: Remove MAC configuration. Read it from the database.
...
func (r *Router) OnPacketIn(finder network.Finder, ingress *network.Port, eth *protocol.Ethernet) error {
	r.log.Debug(fmt.Sprintf("Router: PACKET_IN.. Ingress=%v (MAC=%v), SrcMAC=%v, DstMAC=%v", ingress.ID(), ingress.Value().MAC(), eth.SrcMAC, eth.DstMAC))

	// Is this packet going to the router?
	if bytes.Compare(eth.DstMAC, r.mac) != 0 || bytes.Compare(ingress.Value().MAC(), eth.DstMAC) != 0 {
		r.log.Debug(fmt.Sprintf("Router: ignore PACKET_IN.. Ingress=%v, SrcMAC=%v, DstMAC=%v", ingress.ID(), eth.SrcMAC, eth.DstMAC))
	...
	ok, err = r.db.IsGateway(eth.SrcMAC)
	if err != nil {
		return fmt.Errorf("checking packet's origin: %v", err)
	}
	if ok {
